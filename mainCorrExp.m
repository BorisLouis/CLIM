...
file.path = 'D:\Documents\2025 - Data\Temperature sweep';
file.ext  = '.tif';
...
info.threshold = 0.15; %only used if info.thresholdMode is "fixed"
...
info.frame2Process = 100:1100; %number of frame to used for correlation analysis.
...
save(fileName,'myExperiment','-v7.3');
...
for i =1:size(myExperiment.corrMasks,2)
    ...
    subplot(2,length(myExperiment.corrMasks)/2,i)
    imagesc(myExperiment.corrMasks(i).mask)
    axis image
    colormap('hot')
end
...
for i =1:size(myExperiment.corrMasks,2)
     ...
     subplot(2,length(myExperiment.corrMasks)/2,i)
    imagesc(myExperiment.corrMasks(i).data)
    axis image
    colormap('hot')
    drawnow;
...
for i =1:size(myExperiment.corrMasks,2)
    ...
    subplot(2,length(myExperiment.corrMasks)/2,i)
    imagesc(myExperiment.corrMasks(i).map)
    axis image
    colormap('hot')
    drawnow;
