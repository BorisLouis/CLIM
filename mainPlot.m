...
%% Make map/mask plot
ROI = corrOutput.ROIUsed;
f1 = figure('Position',[250 100 1200 800]);
subplot(1,3,1)
if ~isempty(ROI)
    im = corrOutput.Image(ROI(2):ROI(2)+ROI(4)-1,ROI(1):ROI(1)+ROI(3)-1);
else
    im = corrOutput.Image;
end
imagesc(im);
...
RGBIM = label2rgb(corrOutput.corrMask,'colorcube','k','shuffle');
imagesc(RGBIM);
...
subplot(1,2,1)
hold on
plotContour(f2,im,corrOutput.corrMask);
colormap(gca,'gray')
