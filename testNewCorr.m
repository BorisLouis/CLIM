...
[corrRelation] = myMovie.getPxCorrelation(ROICorrData);


thresh = 0.4;
[corrMask]     = corrAnalysis.corrClusteringV5(corrRelation,ROICorrData,thresh);
myMovie.corrMask.raw = corrMask;
myMovie.corrMask.corrThresh = thresh;
myMovie.corrMask.rawNCluster = numel(unique(corrMask));
myMovie.corrMask.method = 'Seedling';
%% evaluate

[bestClustEval1,bestRelNum] = myMovie.evalCluster(corrMask,ROICorrData);
[cleanMask,silMap] = myMovie.cleanCorrMask(ROICorrData);

%% merge similar cluster
traces = myMovie.getAllTraces(ROICorrData);
